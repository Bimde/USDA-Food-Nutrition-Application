...
public class Database {
	...
	// TODO SEARCH METHOD MUST RETURN A LINKEDLIST OF RESULTS
	// TODO BINARYTREE RETURNED MUST BE ABLE TO BE SEARCHED BY CATEGORY (ex. low
	// fat)
	public Database() {
		long time = System.currentTimeMillis();
		try {
			this.main = Parser.parse("FOOD_DES.txt", FoodBinaryTree.FOOD_DES);
		} catch (Exception e) {
		...
		System.out.println((System.currentTimeMillis() - time) / 1000.0);
		try {
			this.nutrientDef = Parser.parse("NUTR_DEF.txt", FoodBinaryTree.NUTR_DEF);
		} catch (Exception e) {
			System.err.println("Error loading 'NUTR_DEF.txt'");
			e.printStackTrace();
		}
		try {
			this.foodGroups = Parser.parse("FD_GROUP.txt", FoodBinaryTree.FD_GROUP);
		} catch (Exception e) {
	...
	/**
	 * Finds all items in tree which contain specified query RETURNS NULL IF NO
	 * ITEMS ARE FOUND
	 * 
	 * @param query
	...
	public LinkedList<FoodPacket> search(String query) {
		return this.main.search(query, new String[]{"Long_Desc", "Short_Desc", "ComName"});
	}
